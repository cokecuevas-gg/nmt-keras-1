...
from nmt_keras.encoders import *
...
class TranslationModel(Model_Wrapper):
    ...
    def setOptimizer(self, **kwargs):
        ...
        model_to_compile2.compile(optimizer=optimizer,
                                 loss=self.params['LOSS'],
                                 metrics=self.params.get('KERAS_METRICS', []),
                                 loss_weights=self.params.get('LOSS_WEIGHTS', None),
                                 sample_weight_mode='temporal' if self.params['SAMPLE_WEIGHTS'] else None,
    ...
    def ParalelDecoder(self,params):
        # 1. Source text input
        src_text = Input(name=self.ids_inputs[0], batch_shape=tuple([None, None]), dtype='int32')
        # 2. Encoder
        # 2.1. Source word embedding
        ...
        # 3.1.1. Previously generated words as inputs for training -> Teacher forcing
        next_words = Input(name=self.ids_inputs[1], batch_shape=tuple([None, None]), dtype='int32')
        next_words_2 = Input(name=self.ids_inputs[1], batch_shape=tuple([None, None]), dtype='int32')
        # 3.1.2. Target word embedding
        if params.get('TIE_EMBEDDINGS', False):
            state_below = embedding(next_words)
            state_below_2 = embedding(next_words_2)
        else:
            state_below = Embedding(params['OUTPUT_VOCABULARY_SIZE'], params['TARGET_TEXT_EMBEDDING_SIZE'],
                                    name='target_word_embedding',
                                    embeddings_regularizer=l2(params['WEIGHT_DECAY']),
            ...
            state_below_2 = Embedding(params['OUTPUT_VOCABULARY_SIZE_2'], params['TARGET_TEXT_EMBEDDING_SIZE'],
                                    ...
                                    trainable=self.trg_embedding_weights_trainable,
                                    weights=self.trg_embedding_weights,
                                    mask_zero=True)(next_words_2)

        if params.get('SCALE_TARGET_WORD_EMBEDDINGS', False):
            state_below = SqrtScaling(params['TARGET_TEXT_EMBEDDING_SIZE'])(state_below)
            state_below_2 = SqrtScaling(params['TARGET_TEXT_EMBEDDING_SIZE'])(state_below_2)
        state_below = Regularize(state_below, params, name='state_below')
        state_below_2 = Regularize(state_below_2, params, name='state_below')
        ...
        if len(params['INIT_LAYERS']) > 0:
            ...
            initial_state = Regularize(initial_state, params, name='initial_state')
            input_attentional_decoder = [state_below, annotations, initial_state]
            input_attentional_decoder_2 = [state_below_2, annotations, initial_state]
            ...
            if 'LSTM' in params['DECODER_RNN_TYPE']:
                initial_memory = Dense(params['DECODER_HIDDEN_SIZE'], name='initial_memory',
                                       ...
                                       trainable=params.get('TRAINABLE_DECODER', True),
                                       activation=params['INIT_LAYERS'][-1])(ctx_mean)
                initial_memory_2 = Dense(params['DECODER_HIDDEN_SIZE'], name='initial_memory',
                                       kernel_initializer=params['INIT_FUNCTION'],
                                       kernel_regularizer=l2(params['WEIGHT_DECAY']),
                                       bias_regularizer=l2(params['WEIGHT_DECAY']),
                                       trainable=params.get('TRAINABLE_DECODER', True),
                ...
                initial_memory = Regularize(initial_memory, params, name='initial_memory')
                input_attentional_decoder.append(initial_memory)
                input_attentional_decoder_2.append(initial_memory)
        else:
            # Initialize to zeros vector
            input_attentional_decoder = [state_below, annotations]
            input_attentional_decoder_2 = [state_below_2, annotations]
            initial_state = ZeroesLayer(params['DECODER_HIDDEN_SIZE'])(ctx_mean)
            initial_state_2 = ZeroesLayer(params['DECODER_HIDDEN_SIZE'])(ctx_mean)
            input_attentional_decoder.append(initial_state)
            input_attentional_decoder_2.append(initial_state_2)
            if 'LSTM' in params['DECODER_RNN_TYPE']:
                input_attentional_decoder.append(initial_state)
                input_attentional_decoder_2.append(initial_state_2)
        ...
        sharedAttRNNCond_2 = eval('Att' + params['DECODER_RNN_TYPE'] + 'Cond')(params['DECODER_HIDDEN_SIZE'],
                                                                             attention_mode=params.get('ATTENTION_MODE', 'add'),
                                                                             att_units=params.get('ATTENTION_SIZE', 0),
                                                                             kernel_regularizer=l2(params['RECURRENT_WEIGHT_DECAY']),
                                                                             recurrent_regularizer=l2(params['RECURRENT_WEIGHT_DECAY']),
                                                                             ...
                                                                             return_extra_variables=True,
                                                                             return_states=True,
                                                                             num_inputs=len(input_attentional_decoder_2),
                                                                             name='decoder_Att' + params['DECODER_RNN_TYPE'] + 'Cond')

        rnn_output_2 = sharedAttRNNCond_2(input_attentional_decoder_2)
        proj_h_2 = rnn_output_2[0]
        x_att_2 = rnn_output_2[1]
        alphas_2 = rnn_output_2[2]
        h_state_2 = rnn_output_2[3]

        if 'LSTM' in params['DECODER_RNN_TYPE']:
            h_memory = rnn_output[4]
            h_memory_2 = rnn_output_2[4]
        shared_Lambda_Permute = PermuteGeneral((1, 0, 2))
        shared_Lambda_Permute_2 = PermuteGeneral((1, 0, 2))

        if params['DOUBLE_STOCHASTIC_ATTENTION_REG'] > 0:
            alpha_regularizer = AlphaRegularizer(alpha_factor=params['DOUBLE_STOCHASTIC_ATTENTION_REG'])(alphas)
            alpha_regularizer_2 = AlphaRegularizer(alpha_factor=params['DOUBLE_STOCHASTIC_ATTENTION_REG'])(alphas_2)

        [proj_h, shared_reg_proj_h] = Regularize(proj_h, params, shared_layers=True, name='proj_h0')
        [proj_h_2, shared_reg_proj_h_2] = Regularize(proj_h_2, params, shared_layers=True, name='proj_h0')

        # 3.4. Possibly deep decoder
        shared_proj_h_list = []
        shared_proj_h_list_2 = []
        shared_reg_proj_h_list = []
        shared_reg_proj_h_list_2 = []

        h_states_list = [h_state]
        h_states_list_2 = [h_state_2]
        if 'LSTM' in params['DECODER_RNN_TYPE']:
            h_memories_list = [h_memory]
            h_memories_list_2 = [h_memory_2]

        for n_layer in range(1, params['N_LAYERS_DECODER']):
            current_rnn_input = [proj_h, shared_Lambda_Permute(x_att), initial_state]
            current_rnn_input_2 = [proj_h_2, shared_Lambda_Permute_2(x_att_2), initial_state]
            if 'LSTM' in params['DECODER_RNN_TYPE']:
                current_rnn_input.append(initial_memory)
                current_rnn_input_2.append(initial_memory)
            shared_proj_h_list.append(eval(params['DECODER_RNN_TYPE'].replace('Conditional', '') + 'Cond')(
                params['DECODER_HIDDEN_SIZE'],
                kernel_regularizer=l2(params['RECURRENT_WEIGHT_DECAY']),
                recurrent_regularizer=l2(params['RECURRENT_WEIGHT_DECAY']),
            ...
            shared_proj_h_list_2.append(eval(params['DECODER_RNN_TYPE'].replace('Conditional', '') + 'Cond')(
                params['DECODER_HIDDEN_SIZE'],
                kernel_regularizer=l2(params['RECURRENT_WEIGHT_DECAY']),
                recurrent_regularizer=l2(params['RECURRENT_WEIGHT_DECAY']),
                conditional_regularizer=l2(params['RECURRENT_WEIGHT_DECAY']),
                ...
                return_states=True,
                trainable=params.get('TRAINABLE_DECODER', True),
                num_inputs=len(current_rnn_input_2),
                name='decoder_' + params['DECODER_RNN_TYPE'].replace('Conditional', '') + 'Cond' + str(n_layer)))

            current_rnn_output = shared_proj_h_list[-1](current_rnn_input)
            current_rnn_output_2 = shared_proj_h_list_2[-1](current_rnn_input_2)
            current_proj_h = current_rnn_output[0]
            current_proj_h_2 = current_rnn_output_2[0]
            h_states_list.append(current_rnn_output[1])
            h_states_list_2.append(current_rnn_output_2[1])
            if 'LSTM' in params['DECODER_RNN_TYPE']:
                h_memories_list.append(current_rnn_output[2])
                h_memories_list_2.append(current_rnn_output_2[2])
            [current_proj_h, shared_reg_proj_h] = Regularize(current_proj_h, params, shared_layers=True,
                                                             name='proj_h' + str(n_layer))
            [current_proj_h_2, shared_reg_proj_h_2] = Regularize(current_proj_h_2, params, shared_layers=True,
                                                             name='proj_h' + str(n_layer))
            shared_reg_proj_h_list.append(shared_reg_proj_h)
            shared_reg_proj_h_list_2.append(shared_reg_proj_h_2)
        ...
        shared_FC_mlp = TimeDistributed(Dense(params['SKIP_VECTORS_HIDDEN_SIZE'],
                                        ...
                                        trainable=params.get('TRAINABLE_DECODER', True),
                                        name='logit_lstm')
        shared_FC_mlp_2 = TimeDistributed(Dense(params['SKIP_VECTORS_HIDDEN_SIZE'],
                                              kernel_initializer=params['INIT_FUNCTION'],
                                              kernel_regularizer=l2(params['WEIGHT_DECAY']),
                                              bias_regularizer=l2(params['WEIGHT_DECAY']),
                                              trainable=params.get('TRAINABLE_DECODER', True),
        ...
        shared_FC_ctx = TimeDistributed(Dense(params['SKIP_VECTORS_HIDDEN_SIZE'],
                                        ...
                                        trainable=params.get('TRAINABLE_DECODER', True),
                                        name='logit_ctx')
        shared_FC_ctx_2 = TimeDistributed(Dense(params['SKIP_VECTORS_HIDDEN_SIZE'],
                                              kernel_initializer=params['INIT_FUNCTION'],
                                              kernel_regularizer=l2(params['WEIGHT_DECAY']),
                                              bias_regularizer=l2(params['WEIGHT_DECAY']),
                                              trainable=params.get('TRAINABLE_DECODER', True),
                                        ...
                                        name='logit_ctx')
        out_layer_ctx = shared_FC_ctx(x_att)
        out_layer_ctx_2 = shared_FC_ctx_2(x_att_2)
        out_layer_ctx = shared_Lambda_Permute(out_layer_ctx)
        out_layer_ctx_2 = shared_Lambda_Permute_2(out_layer_ctx_2)
        shared_FC_emb = TimeDistributed(Dense(params['SKIP_VECTORS_HIDDEN_SIZE'],
                                              kernel_initializer=params['INIT_FUNCTION'],
                                              kernel_regularizer=l2(params['WEIGHT_DECAY']),
                                              bias_regularizer=l2(params['WEIGHT_DECAY']),
                                        ...
                                        trainable=params.get('TRAINABLE_DECODER', True),
                                        name='logit_emb')
        shared_FC_emb_2 = TimeDistributed(Dense(params['SKIP_VECTORS_HIDDEN_SIZE'],
                                              kernel_initializer=params['INIT_FUNCTION'],
                                              kernel_regularizer=l2(params['WEIGHT_DECAY']),
                                              bias_regularizer=l2(params['WEIGHT_DECAY']),
                                              trainable=params.get('TRAINABLE_DECODER', True),
                                        ...
                                        name='logit_emb')    
        out_layer_emb = shared_FC_emb(state_below)
        out_layer_emb_2 = shared_FC_emb_2(state_below_2)

        [out_layer_mlp, shared_reg_out_layer_mlp] = Regularize(out_layer_mlp, params,
                                                               shared_layers=True, name='out_layer_mlp')
        [out_layer_mlp_2, shared_reg_out_layer_mlp_2] = Regularize(out_layer_mlp_2, params,
                                                               shared_layers=True, name='out_layer_mlp')
        [out_layer_ctx, shared_reg_out_layer_ctx] = Regularize(out_layer_ctx, params,
                                                               shared_layers=True, name='out_layer_ctx')
        [out_layer_ctx_2, shared_reg_out_layer_ctx_2] = Regularize(out_layer_ctx_2, params,
                                                               shared_layers=True, name='out_layer_ctx')
        [out_layer_emb, shared_reg_out_layer_emb] = Regularize(out_layer_emb, params,
                                                               shared_layers=True, name='out_layer_emb')
        [out_layer_emb_2, shared_reg_out_layer_emb_2] = Regularize(out_layer_emb_2, params,
                                                               shared_layers=True, name='out_layer_emb')

        shared_additional_output_merge = eval(params['ADDITIONAL_OUTPUT_MERGE_MODE'])(name='additional_input')
        shared_additional_output_merge_2 = eval(params['ADDITIONAL_OUTPUT_MERGE_MODE'])(name='additional_input')
        additional_output = shared_additional_output_merge([out_layer_mlp, out_layer_ctx, out_layer_emb])
        additional_output_2 = shared_additional_output_merge_2([out_layer_mlp_2, out_layer_ctx_2, out_layer_emb_2])
        shared_activation = Activation(params.get('SKIP_VECTORS_SHARED_ACTIVATION', 'tanh'))
        shared_activation_2 = Activation(params.get('SKIP_VECTORS_SHARED_ACTIVATION', 'tanh'))

        out_layer = shared_activation(additional_output)
        out_layer_2 = shared_activation_2(additional_output_2)

        shared_deep_list = []
        shared_deep_list_2 = []
        shared_reg_deep_list = []
        shared_reg_deep_list_2 = []
        # 3.6 Optional deep ouput layer
        for i, (activation, dimension) in enumerate(params['DEEP_OUTPUT_LAYERS']):
            shared_deep_list.append(TimeDistributed(Dense(dimension, activation=activation,
                                                          kernel_initializer=params['INIT_FUNCTION'],
                                                    ...
                                                    trainable=params.get('TRAINABLE_DECODER', True),
                                                    name=activation + '_%d' % i))
            shared_deep_list_2.append(TimeDistributed(Dense(dimension, activation=activation,
                                                          kernel_initializer=params['INIT_FUNCTION'],
                                                          kernel_regularizer=l2(params['WEIGHT_DECAY']),
                                                          bias_regularizer=l2(params['WEIGHT_DECAY']),
                                                          trainable=params.get('TRAINABLE_DECODER', True),
                                                    ...
                                                    name=activation + '_%d' % i))
            out_layer = shared_deep_list[-1](out_layer)
            out_layer_2 = shared_deep_list_2[-1](out_layer_2)
            [out_layer, shared_reg_out_layer] = Regularize(out_layer,
                                                           params, shared_layers=True,
                                                           name='out_layer_' + str(activation) + '_%d' % i)
            [out_layer_2, shared_reg_out_layer_2] = Regularize(out_layer_2,
                                                           params, shared_layers=True,
                                                           name='out_layer_' + str(activation) + '_%d' % i)
            shared_reg_deep_list.append(shared_reg_out_layer)
            shared_reg_deep_list_2.append(shared_reg_out_layer_2)
        ...
        shared_FC_soft_2 = TimeDistributed(Dense(params['OUTPUT_VOCABULARY_SIZE_2'],
                                         ...
                                         name=self.ids_outputs[0])
        softout = shared_FC_soft(out_layer)
        softout_2 = shared_FC_soft_2(out_layer_2)

        self.model = Model(inputs=[src_text, next_words],outputs=softout,name=self.name + '_training_es')
        plot_model(self.model, to_file='model_es.png', show_shapes=True, show_layer_names=True)
        self.model2 = Model(inputs=[src_text, next_words_2],outputs=softout_2,name=self.name + '_training_fr')
        plot_model(self.model2, to_file='model_fr.png', show_shapes=True, show_layer_names=True)
        
        if params['DOUBLE_STOCHASTIC_ATTENTION_REG'] > 0.:
            self.model.add_loss(alpha_regularizer)
            self.model2.add_loss(alpha_regularizer)
        ...
        # for applying the initial forward pass
        model_init_input = [src_text, next_words]
        model_init_input_2 = [src_text, next_words_2]
        model_init_output = [softout, annotations] + h_states_list
        model_init_output_2 = [softout_2, annotations] + h_states_list_2
        if 'LSTM' in params['DECODER_RNN_TYPE']:
            model_init_output += h_memories_list
            model_init_output_2 += h_memories_list_2
        if self.return_alphas:
            model_init_output.append(alphas)
            model_init_output_2.append(alphas_2)
        self.model_init = Model(inputs=model_init_input,
                                outputs=model_init_output,
                                name=self.name + '_model_init')
        
        self.model_init2 = Model(inputs=model_init_input_2,
                                outputs=model_init_output_2,
                                name=self.name + '_model_init')

        # Store inputs and outputs names for model_init
        self.ids_inputs_init = self.ids_inputs
        ids_states_names = ['next_state_' + str(i) for i in range(len(h_states_list))]
        ids_states_names_2 = ['next_state_' + str(i) for i in range(len(h_states_list_2))]

        # first output must be the output probs.
        self.ids_outputs_init = self.ids_outputs + ['preprocessed_input'] + ids_states_names
        self.ids_outputs_init_2 = self.ids_outputs + ['preprocessed_input'] + ids_states_names_2
        if 'LSTM' in params['DECODER_RNN_TYPE']:
            ids_memories_names = ['next_memory_' + str(i) for i in range(len(h_memories_list))]
            ids_memories_names_2 = ['next_memory_' + str(i) for i in range(len(h_memories_list_2))]
            self.ids_outputs_init += ids_memories_names
            self.ids_outputs_init_2 += ids_memories_names_2
        # Second, we need to build an additional model with the capability to have the following inputs:
        #   - preprocessed_input
        #   - prev_word
        #   - prev_state
        ...
        # Define inputs
        n_deep_decoder_layer_idx = 0
        n_deep_decoder_layer_idx_2 = 0
        preprocessed_annotations = Input(name='preprocessed_input', shape=tuple([None, preprocessed_size]))
        preprocessed_annotations_2 = Input(name='preprocessed_input', shape=tuple([None, preprocessed_size]))
        prev_h_states_list = [Input(name='prev_state_' + str(i),
                                    shape=tuple([params['DECODER_HIDDEN_SIZE']]))
                              for i in range(len(h_states_list))]
        prev_h_states_list_2 = [Input(name='prev_state_' + str(i),
                                    shape=tuple([params['DECODER_HIDDEN_SIZE']]))
                              for i in range(len(h_states_list_2))]

        input_attentional_decoder = [state_below, preprocessed_annotations,
                                     prev_h_states_list[n_deep_decoder_layer_idx]]
        
        input_attentional_decoder_2 = [state_below_2, preprocessed_annotations_2,
                                     prev_h_states_list_2[n_deep_decoder_layer_idx_2]]

        if 'LSTM' in params['DECODER_RNN_TYPE']:
            prev_h_memories_list = [Input(name='prev_memory_' + str(i),
                                          shape=tuple([params['DECODER_HIDDEN_SIZE']]))
                                    for i in range(len(h_memories_list))]
            prev_h_memories_list_2 = [Input(name='prev_memory_' + str(i),
                                          shape=tuple([params['DECODER_HIDDEN_SIZE']]))
                                    for i in range(len(h_memories_list_2))]

            input_attentional_decoder.append(prev_h_memories_list[n_deep_decoder_layer_idx])
            input_attentional_decoder_2.append(prev_h_memories_list_2[n_deep_decoder_layer_idx_2])
        # Apply decoder
        rnn_output = sharedAttRNNCond(input_attentional_decoder)
        rnn_output_2 = sharedAttRNNCond_2(input_attentional_decoder_2)
        proj_h = rnn_output[0]
        ...
        x_att_2 = rnn_output_2[1]
        alphas_2 = rnn_output_2[2]
        h_states_list_2 = [rnn_output_2[3]]

        if 'LSTM' in params['DECODER_RNN_TYPE']:
            h_memories_list = [rnn_output[4]]
            h_memories_list_2 = [rnn_output_2[4]]
        for reg in shared_reg_proj_h:
            proj_h = reg(proj_h)
        for reg in shared_reg_proj_h_2:
            proj_h_2 = reg(proj_h_2)
        ...
        for (rnn_decoder_layer, proj_h_reg) in zip(shared_proj_h_list_2, shared_reg_proj_h_list_2):
            n_deep_decoder_layer_idx_2 += 1
            input_rnn_decoder_layer_2 = [proj_h_2, shared_Lambda_Permute_2(x_att_2),
                                       prev_h_states_list_2[n_deep_decoder_layer_idx_2]]
            if 'LSTM' in params['DECODER_RNN_TYPE']:
        ...
        for (reg_out_layer_mlp, reg_out_layer_ctx, reg_out_layer_emb) in zip(shared_reg_out_layer_mlp_2,
                                                                             shared_reg_out_layer_ctx_2,
                                                                             shared_reg_out_layer_emb_2):
            out_layer_mlp_2 = reg_out_layer_mlp(out_layer_mlp_2)
            out_layer_ctx_2= reg_out_layer_ctx(out_layer_ctx_2)
            out_layer_emb_2 = reg_out_layer_emb(out_layer_emb_2)
        ...
        for (deep_out_layer, reg_list) in zip(shared_deep_list, shared_reg_deep_list):
            ...
            for reg in reg_list:
                out_layer = reg(out_layer)
        for (deep_out_layer, reg_list) in zip(shared_deep_list_2, shared_reg_deep_list_2):
            out_layer_2 = deep_out_layer(out_layer_2)
            for reg in reg_list:
                out_layer_2 = reg(out_layer_2)

        # Softmax
        softout = shared_FC_soft(out_layer)
        softout_2 = shared_FC_soft_2(out_layer_2)
        model_next_inputs = [next_words, preprocessed_annotations] + prev_h_states_list
        model_next_inputs_2 = [next_words_2, preprocessed_annotations_2] + prev_h_states_list_2
        model_next_outputs = [softout, preprocessed_annotations] + h_states_list
        model_next_outputs_2 = [softout_2, preprocessed_annotations_2] + h_states_list_2
        if 'LSTM' in params['DECODER_RNN_TYPE']:
            model_next_inputs += prev_h_memories_list
            model_next_inputs_2 += prev_h_memories_list_2
            model_next_outputs += h_memories_list
            model_next_outputs_2 += h_memories_list_2

        if self.return_alphas:
            model_next_outputs.append(alphas)
            model_next_outputs_2.append(alphas_2)

        self.model_next = Model(inputs=model_next_inputs,outputs=model_next_outputs,name=self.name + '_model_next')
        self.model_next2 = Model(inputs=model_next_inputs_2,outputs=model_next_outputs_2,name=self.name + '_model_next')
        plot_model(self.model_next, to_file='model_next.png', show_shapes=True, show_layer_names=True)
        # Store inputs and outputs names for model_next
        # first input must be previous word
        self.ids_inputs_next = [self.ids_inputs[1]] + ['preprocessed_input']
        self.ids_inputs_next_2 = [self.ids_inputs[1]] + ['preprocessed_input']
        # first output must be the output probs.
        self.ids_outputs_next = self.ids_outputs + ['preprocessed_input']
        self.ids_outputs_next_2 = self.ids_outputs + ['preprocessed_input']
        # Input -> Output matchings from model_init to model_next and from model_next to model_next
        self.matchings_init_to_next = {'preprocessed_input': 'preprocessed_input'}
        self.matchings_init_to_next_2 = {'preprocessed_input': 'preprocessed_input'}
        self.matchings_next_to_next = {'preprocessed_input': 'preprocessed_input'}
        self.matchings_next_to_next_2 = {'preprocessed_input': 'preprocessed_input'}
        # append all next states and matchings
        ...
        for n_state in range(len(prev_h_states_list_2)):
            self.ids_inputs_next_2.append('prev_state_' + str(n_state))
            self.ids_outputs_next_2.append('next_state_' + str(n_state))
            self.matchings_init_to_next_2['next_state_' + str(n_state)] = 'prev_state_' + str(n_state)
            self.matchings_next_to_next_2['next_state_' + str(n_state)] = 'prev_state_' + str(n_state)
        ...
        if 'LSTM' in params['DECODER_RNN_TYPE']:
            for n_memory in range(len(prev_h_memories_list)):
                ...
                self.matchings_init_to_next['next_memory_' + str(n_memory)] = 'prev_memory_' + str(n_memory)
                self.matchings_next_to_next['next_memory_' + str(n_memory)] = 'prev_memory_' + str(n_memory)       
            for n_memory in range(len(prev_h_memories_list_2)):
                self.ids_inputs_next_2.append('prev_memory_' + str(n_memory))
                self.ids_outputs_next_2.append('next_memory_' + str(n_memory))
                self.matchings_init_to_next_2['next_memory_' + str(n_memory)] = 'prev_memory_' + str(n_memory)
                self.matchings_next_to_next_2['next_memory_' + str(n_memory)] = 'prev_memory_' + str(n_memory)       
