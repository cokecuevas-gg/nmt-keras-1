...
class TranslationModel(Model_Wrapper):
    ...
    def AttentionRNNEncoderDecoder(self, params):
        ...
        if params['MULTILANGUAGE'] == 1:
            next_words = Input(name=self.ids_inputs[1]+"_1", batch_shape=tuple([None, None]), dtype='int32')
        else:
        ...
        shared_encoder_model = Model(name='Shared Encoder',inputs=src_text,outputs=[annotations,initial_state,initial_memory])
        plot_model(shared_encoder_model, to_file='encoder_original.png', show_shapes=True, show_layer_names=True)
        # 3.3. Attentional decoder
        #AttConditionalLSTMCond
        ...
        # se creará una nueva instancia con un nuevo state belo
        #o debo utilizar el mismo objetov?
        rnn_output = sharedAttRNNCond(input_attentional_decoder)
        rnn_output2 = sharedAttRNNCond(input_attentional_decoder)
        proj_h = rnn_output[0]
        x_att = rnn_output[1]
        alphas = rnn_output[2]
        ...
        plot_model(self.model, to_file='encoder.png', show_shapes=True, show_layer_names=True)
        if params['DOUBLE_STOCHASTIC_ATTENTION_REG'] > 0.:
    ...
    def AttentionRNNEncoderNonSharedDecoder(self,params):
        # 1. Source text input
        src_text = Input(name=self.ids_inputs[0], batch_shape=tuple([None, None]), dtype='int32')
        # 2. Encoder
        # 2.1. Source word embedding
        ...
        # 3. Decoder
        # 3.1.1. Previously generated words as inputs for training -> Teacher forcing
        next_words = Input(name='state_below_0', batch_shape=tuple([None, None]), dtype='int32')
        next_words2 = Input(name='state_below_1', batch_shape=tuple([None, None]), dtype='int32')
        # 3.1.2. Target word embedding
        state_below_es = Embedding(params['OUTPUT_VOCABULARY_SIZE'], params['TARGET_TEXT_EMBEDDING_SIZE'],
                                    name='target_word_embedding_es',
                                    embeddings_regularizer=l2(params['WEIGHT_DECAY']),
                                    embeddings_initializer=params['INIT_FUNCTION'],
                                    trainable=self.trg_embedding_weights_trainable,
                                    weights=self.trg_embedding_weights,
                                    mask_zero=True)(next_words)
        state_below_fr = Embedding(params['OUTPUT_VOCABULARY_SIZE'], params['TARGET_TEXT_EMBEDDING_SIZE'],
                                    name='target_word_embedding_fr',
                                    embeddings_regularizer=l2(params['WEIGHT_DECAY']),
                                    embeddings_initializer=params['INIT_FUNCTION'],
                                    trainable=self.trg_embedding_weights_trainable,
                                    weights=self.trg_embedding_weights,
                                    mask_zero=True)(next_words2)
        state_below_es = Regularize(state_below_es, params, name='state_below_es')
        state_below_fr = Regularize(state_below_fr, params, name='state_below_fr')
        # 3.2. Decoder's RNN initialization perceptrons with ctx mean
        annotations = ApplyMask(name='annotations')([annotations, src_embedding_mask])  # We may want the padded annotations
        ctx_mean = MaskedMean(name='ctx_mean')(annotations)
        ...
        if len(params['INIT_LAYERS']) > 0:
            ...
            initial_state = Regularize(initial_state, params, name='initial_state')
            input_attentional_decoder_es = [state_below_es, annotations, initial_state]
            input_attentional_decoder_fr = [state_below_fr, annotations, initial_state]
            ...
            if 'LSTM' in params['DECODER_RNN_TYPE']:
                initial_memory = Dense(params['DECODER_HIDDEN_SIZE'], name='initial_memory',
                                       ...
                                       activation=params['INIT_LAYERS'][-1])(ctx_mean)
                initial_memory = Regularize(initial_memory, params, name='initial_memory')
                input_attentional_decoder_es.append(initial_memory)
                input_attentional_decoder_fr.append(initial_memory)
        else:
            # Initialize to zeros vector
            input_attentional_decoder_es = [state_below_es, shared_encoder_model.get_layer("ctx_mean").output]
            input_attentional_decoder_fr = [state_below_fr, shared_encoder_model.get_layer("ctx_mean").output]
            initial_state = ZeroesLayer(params['DECODER_HIDDEN_SIZE'])(ctx_mean)
            input_attentional_decoder_es.append(initial_state)
            input_attentional_decoder_fr.append(initial_state)
            if 'LSTM' in params['DECODER_RNN_TYPE']:
                input_attentional_decoder_es.append(initial_state)
                input_attentional_decoder_fr.append(initial_state)
        shared_encoder_model = Model(name='Shared Encoder',inputs=src_text,outputs=[annotations,initial_state,initial_memory])
        #plot_model(shared_encoder_model, to_file='encoder_shared.png', show_shapes=True, show_layer_names=True)
        input_attentional_decoder_es = [state_below_es,shared_encoder_model.get_layer("annotations").output,shared_encoder_model.get_layer("initial_state_batch_normalization").output,shared_encoder_model.get_layer("initial_memory_batch_normalization").output]
        input_attentional_decoder_fr = [state_below_fr,shared_encoder_model.get_layer("annotations").output,shared_encoder_model.get_layer("initial_state_batch_normalization").output,shared_encoder_model.get_layer("initial_memory_batch_normalization").output]
        sharedAttRNNCond_es = eval('Att' + params['DECODER_RNN_TYPE'] + 'Cond')(params['DECODER_HIDDEN_SIZE'],
                                                                             attention_mode=params.get('ATTENTION_MODE', 'add'),
                                                                             att_units=params.get('ATTENTION_SIZE', 0),
                                                                             kernel_regularizer=l2(params['RECURRENT_WEIGHT_DECAY']),
                                                                             recurrent_regularizer=l2(params['RECURRENT_WEIGHT_DECAY']),
                                                                             ...
                                                                             return_extra_variables=True,
                                                                             return_states=True,
                                                                             num_inputs=len(input_attentional_decoder_es),
                                                                             name='es_decoder_Att' + params['DECODER_RNN_TYPE'] + 'Cond')
        [proj_h_es, x_att_es, alphas_es, h_state_es,h_memory_es] = sharedAttRNNCond_es(input_attentional_decoder_es) 
        sharedAttRNNCond_fr = eval('Att' + params['DECODER_RNN_TYPE'] + 'Cond')(params['DECODER_HIDDEN_SIZE'],
                                                                             attention_mode=params.get('ATTENTION_MODE', 'add'),
                                                                             att_units=params.get('ATTENTION_SIZE', 0),
                                                                             kernel_regularizer=l2(params['RECURRENT_WEIGHT_DECAY']),
                                                                             recurrent_regularizer=l2(params['RECURRENT_WEIGHT_DECAY']),
                                                                             ...
                                                                             return_extra_variables=True,
                                                                             return_states=True,
                                                                             num_inputs=len(input_attentional_decoder_fr),
                                                                             name='fr_decoder_Att' + params['DECODER_RNN_TYPE'] + 'Cond')    
        [proj_h_fr, x_att_fr, alphas_fr, h_state_fr,h_memory_fr] = sharedAttRNNCond_fr(input_attentional_decoder_fr) 
        for l in [proj_h_fr, x_att_fr, alphas_fr, h_state_fr,h_memory_fr]:
            print(l)
        shared_Lambda_Permute_es = PermuteGeneral((1, 0, 2))
        shared_Lambda_Permute_fr = PermuteGeneral((1, 0, 2))
        # 3.4. Possibly deep decoder
        [proj_h_es, shared_reg_proj_h_es] = Regularize(proj_h_es, params, shared_layers=True, name='proj_h0_es')
        [proj_h_fr, shared_reg_proj_h_fr] = Regularize(proj_h_fr, params, shared_layers=True, name='proj_h0_fr')
        
        shared_proj_h_list_es = []
        shared_reg_proj_h_list_es = []
        shared_proj_h_list_fr = []
        shared_reg_proj_h_list_fr = []

        h_states_list_es = [h_state_es]
        h_states_list_fr = [h_state_fr]
        if 'LSTM' in params['DECODER_RNN_TYPE']:
            h_memories_list_es = [h_memory_es]
            h_memories_list_fr = [h_memory_fr]

        for n_layer in range(1, params['N_LAYERS_DECODER']):
            current_rnn_input_es = [proj_h_es, shared_Lambda_Permute_es(x_att_es), initial_state]
            current_rnn_input_fr = [proj_h_fr, shared_Lambda_Permute_es(x_att_fr), initial_state]
            if 'LSTM' in params['DECODER_RNN_TYPE']:
                current_rnn_input_es.append(initial_memory)
                current_rnn_input_fr.append(initial_memory)
            shared_proj_h_list_es.append(eval(params['DECODER_RNN_TYPE'].replace('Conditional', '') + 'Cond')(
                params['DECODER_HIDDEN_SIZE'],
                kernel_regularizer=l2(params['RECURRENT_WEIGHT_DECAY']),
                recurrent_regularizer=l2(params['RECURRENT_WEIGHT_DECAY']),
                conditional_regularizer=l2(params['RECURRENT_WEIGHT_DECAY']),
                ...
                return_states=True,
                trainable=params.get('TRAINABLE_DECODER', True),
                num_inputs=len(current_rnn_input_es),
                name='es_decoder_' + params['DECODER_RNN_TYPE'].replace('Conditional', '') + 'Cond' + str(n_layer)))
            
            shared_proj_h_list_fr.append(eval(params['DECODER_RNN_TYPE'].replace('Conditional', '') + 'Cond')(
                params['DECODER_HIDDEN_SIZE'],
                kernel_regularizer=l2(params['RECURRENT_WEIGHT_DECAY']),
                recurrent_regularizer=l2(params['RECURRENT_WEIGHT_DECAY']),
                conditional_regularizer=l2(params['RECURRENT_WEIGHT_DECAY']),
                ...
                return_states=True,
                trainable=params.get('TRAINABLE_DECODER', True),
                num_inputs=len(current_rnn_input_fr),
                name='fr_decoder_' + params['DECODER_RNN_TYPE'].replace('Conditional', '') + 'Cond' + str(n_layer)))

            current_rnn_output_es = shared_proj_h_list_es[-1](current_rnn_input_es)
            current_rnn_output_fr = shared_proj_h_list_fr[-1](current_rnn_input_fr)
            current_proj_h_es = current_rnn_output_es[0]
            current_proj_h_fr = current_rnn_output_fr[0]
            h_states_list_es.append(current_rnn_output_es[1])
            h_states_list_fr.append(current_rnn_output_fr[1])
            if 'LSTM' in params['DECODER_RNN_TYPE']:
                h_memories_list_es.append(current_rnn_output_es[2])
                h_memories_list_fr.append(current_rnn_output_fr[2])
            [current_proj_h_es, shared_reg_proj_h_es] = Regularize(current_proj_h_es, params, shared_layers=True,
                                                             name='proj_h' + str(n_layer))
            [current_proj_h_fr, shared_reg_proj_h_fr] = Regularize(current_proj_h_fr, params, shared_layers=True,
                                                             name='proj_h' + str(n_layer))
            shared_reg_proj_h_list_es.append(shared_reg_proj_h_es)
            shared_reg_proj_h_list_fr.append(shared_reg_proj_h_fr)

            proj_h_es = Add()([proj_h_es, current_proj_h_es])
            proj_h_fr = Add()([proj_h_fr, current_proj_h_fr])

        shared_FC_mlp_es = TimeDistributed(Dense(params['SKIP_VECTORS_HIDDEN_SIZE'],
                                              kernel_initializer=params['INIT_FUNCTION'],
                                              kernel_regularizer=l2(params['WEIGHT_DECAY']),
                                              bias_regularizer=l2(params['WEIGHT_DECAY']),
                                              trainable=params.get('TRAINABLE_DECODER', True),
                                        ...
                                        trainable=params.get('TRAINABLE_DECODER', True),
                                        name='es_logit_lstm')
        out_layer_mlp_es = shared_FC_mlp_es(proj_h_es)
        shared_FC_mlp_fr = TimeDistributed(Dense(params['SKIP_VECTORS_HIDDEN_SIZE'],
                                              kernel_initializer=params['INIT_FUNCTION'],
                                              kernel_regularizer=l2(params['WEIGHT_DECAY']),
                                              bias_regularizer=l2(params['WEIGHT_DECAY']),
                                              trainable=params.get('TRAINABLE_DECODER', True),
                                        ...
                                        trainable=params.get('TRAINABLE_DECODER', True),
                                        name='fr_logit_lstm')
        out_layer_mlp_fr = shared_FC_mlp_fr(proj_h_fr)
        shared_FC_ctx_es = TimeDistributed(Dense(params['SKIP_VECTORS_HIDDEN_SIZE'],
                                              kernel_initializer=params['INIT_FUNCTION'],
                                              kernel_regularizer=l2(params['WEIGHT_DECAY']),
                                              bias_regularizer=l2(params['WEIGHT_DECAY']),
                                              trainable=params.get('TRAINABLE_DECODER', True),
                                        ...
                                        trainable=params.get('TRAINABLE_DECODER', True),
                                        name='es_logit_ctx')
        out_layer_ctx_es = shared_FC_ctx_es(x_att_es)
        shared_FC_ctx_fr = TimeDistributed(Dense(params['SKIP_VECTORS_HIDDEN_SIZE'],
                                              kernel_initializer=params['INIT_FUNCTION'],
                                              kernel_regularizer=l2(params['WEIGHT_DECAY']),
                                              bias_regularizer=l2(params['WEIGHT_DECAY']),
                                              trainable=params.get('TRAINABLE_DECODER', True),
                                        ...
                                        trainable=params.get('TRAINABLE_DECODER', True),
                                        name='fr_logit_ctx')
        out_layer_ctx_fr = shared_FC_ctx_fr(x_att_fr)
        out_layer_ctx_es = shared_Lambda_Permute_es(out_layer_ctx_es)
        out_layer_ctx_fr = shared_Lambda_Permute_fr(out_layer_ctx_fr)
        shared_FC_emb_es = TimeDistributed(Dense(params['SKIP_VECTORS_HIDDEN_SIZE'],
                                              kernel_initializer=params['INIT_FUNCTION'],
                                              kernel_regularizer=l2(params['WEIGHT_DECAY']),
                                              bias_regularizer=l2(params['WEIGHT_DECAY']),
                                              trainable=params.get('TRAINABLE_DECODER', True),
                                        ...
                                        trainable=params.get('TRAINABLE_DECODER', True),
                                        name='es_logit_emb')
        shared_FC_emb_fr = TimeDistributed(Dense(params['SKIP_VECTORS_HIDDEN_SIZE'],
                                              kernel_initializer=params['INIT_FUNCTION'],
                                              kernel_regularizer=l2(params['WEIGHT_DECAY']),
                                              bias_regularizer=l2(params['WEIGHT_DECAY']),
                                              trainable=params.get('TRAINABLE_DECODER', True),
                                        ...
                                        trainable=params.get('TRAINABLE_DECODER', True),
                                        name='fr_logit_emb')
        out_layer_emb_es = shared_FC_emb_es(state_below_es)
        out_layer_emb_fr = shared_FC_emb_fr(state_below_fr)

        [out_layer_mlp_es, shared_reg_out_layer_mlp_es] = Regularize(out_layer_mlp_es, params,
                                                               shared_layers=True, name='es_out_layer_mlp')
        [out_layer_ctx_es, shared_reg_out_layer_ctx_es] = Regularize(out_layer_ctx_es, params,
                                                               shared_layers=True, name='es_out_layer_ctx')
        [out_layer_emb_es, shared_reg_out_layer_emb_es] = Regularize(out_layer_emb_es, params,
                                                               shared_layers=True, name='es_out_layer_emb')

        [out_layer_mlp_fr, shared_reg_out_layer_mlp_fr] = Regularize(out_layer_mlp_fr, params,
                                                               shared_layers=True, name='fr_out_layer_mlp')
        [out_layer_ctx_fr, shared_reg_out_layer_ctx_fr] = Regularize(out_layer_ctx_fr, params,
                                                               shared_layers=True, name='fr_out_layer_ctx')
        [out_layer_emb_fr, shared_reg_out_layer_emb_fr] = Regularize(out_layer_emb_fr, params,
                                                               shared_layers=True, name='fr_out_layer_emb')
        shared_additional_output_merge_es = eval(params['ADDITIONAL_OUTPUT_MERGE_MODE'])(name='es_additional_input')
        additional_output_es = shared_additional_output_merge_es([out_layer_mlp_es, out_layer_ctx_es, out_layer_emb_es])
        shared_activation_es = Activation(params.get('SKIP_VECTORS_SHARED_ACTIVATION', 'tanh'))

        shared_additional_output_merge_fr = eval(params['ADDITIONAL_OUTPUT_MERGE_MODE'])(name='fr_additional_input')
        additional_output_fr = shared_additional_output_merge_fr([out_layer_mlp_fr, out_layer_ctx_fr, out_layer_emb_fr])
        shared_activation_fr = Activation(params.get('SKIP_VECTORS_SHARED_ACTIVATION', 'tanh'))

        out_layer_es = shared_activation_es(additional_output_es)
        out_layer_fr = shared_activation_fr(additional_output_fr)

        shared_deep_list_es = []
        shared_reg_deep_list_es = []
        shared_deep_list_fr = []
        shared_reg_deep_list_fr = []
        # 3.6 Optional deep ouput layer
        for i, (activation, dimension) in enumerate(params['DEEP_OUTPUT_LAYERS']):
            shared_deep_list_es.append(TimeDistributed(Dense(dimension, activation=activation,
                                                          kernel_initializer=params['INIT_FUNCTION'],
                                                          kernel_regularizer=l2(params['WEIGHT_DECAY']),
                                                          bias_regularizer=l2(params['WEIGHT_DECAY']),
                                                          trainable=params.get('TRAINABLE_DECODER', True),
                                                    ...
                                                    trainable=params.get('TRAINABLE_DECODER', True),
                                                    name="es_"+activation + '_%d' % i))
            out_layer_es = shared_deep_list_es[-1](out_layer_es)
            [out_layer_es, shared_reg_out_layer_es] = Regularize(out_layer_es,
                                                           params, shared_layers=True,
                                                           name='es_out_layer_' + str(activation) + '_%d' % i)
            shared_reg_deep_list_es.append(shared_reg_out_layer_es)
            shared_deep_list_fr.append(TimeDistributed(Dense(dimension, activation=activation,
                                                          kernel_initializer=params['INIT_FUNCTION'],
                                                          kernel_regularizer=l2(params['WEIGHT_DECAY']),
                                                          bias_regularizer=l2(params['WEIGHT_DECAY']),
                                                          trainable=params.get('TRAINABLE_DECODER', True),
                                                    ...
                                                    trainable=params.get('TRAINABLE_DECODER', True),
                                                    name="fr_"+activation + '_%d' % i))
            out_layer_fr = shared_deep_list_fr[-1](out_layer_fr)
            [out_layer_fr, shared_reg_out_layer_fr] = Regularize(out_layer_fr,
                                                                    params, shared_layers=True,
                                                                    name='fr_out_layer_' + str(activation) + '_%d' % i)
            shared_reg_deep_list_fr.append(shared_reg_out_layer_fr)
        # 3.7. Output layer: Softmax
        shared_FC_soft_es = TimeDistributed(Dense(params['OUTPUT_VOCABULARY_SIZE'],
                                               activation=params['CLASSIFIER_ACTIVATION'],
                                               kernel_regularizer=l2(params['WEIGHT_DECAY']),
                                               bias_regularizer=l2(params['WEIGHT_DECAY']),
                                               trainable=(params.get('TRAINABLE_DECODER', True) or params.get('TRAIN_ONLY_LAST_LAYER', True)),
                                         ...
                                         trainable=(params.get('TRAINABLE_DECODER', True) or params.get('TRAIN_ONLY_LAST_LAYER', True)),
                                         name='target_text_es')
        softout_es = shared_FC_soft_es(out_layer_es)
        shared_FC_soft_fr = TimeDistributed(Dense(params['OUTPUT_VOCABULARY_SIZE'],
                                               activation=params['CLASSIFIER_ACTIVATION'],
                                               kernel_regularizer=l2(params['WEIGHT_DECAY']),
                                               bias_regularizer=l2(params['WEIGHT_DECAY']),
                                               trainable=(params.get('TRAINABLE_DECODER', True) or params.get('TRAIN_ONLY_LAST_LAYER', True)),
                                         ...
                                         trainable=(params.get('TRAINABLE_DECODER', True) or params.get('TRAIN_ONLY_LAST_LAYER', True)),
                                         name='target_text_fr')
        softout_fr = shared_FC_soft_fr(out_layer_fr)
        model = Model(name="Full model",inputs=[src_text,next_words,next_words2],outputs=[softout_es,softout_fr])
        plot_model(model, to_file='model_plot222.png', show_shapes=True, show_layer_names=True)
        self.model = model
