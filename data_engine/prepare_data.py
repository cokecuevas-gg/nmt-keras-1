...
def build_dataset_multilanguage2(params):
    """
    Builds (or loads) a Dataset instance.
    :param params: Parameters specifying Dataset options
    :return: Dataset object
    ...
    if params['REBUILD_DATASET']:  # We build a new dataset instance
        if params['VERBOSE'] > 0:
            silence = False
            logger.info('Building ' + params['DATASET_NAME'] + '_' + str(params['SRCS_LAN']) + str(params['TRG_LAN']) + ' dataset')
        else:
            silence = True

        base_path = params['DATA_ROOT_PATH']
        datasets = []
        for source_l in params['SRCS_LAN']:
            for i,target_l in enumerate(params['TRGS_LAN']): 
                name = params['DATASET_NAME'] + '_' + str(source_l) + str(target_l)
                ds = Dataset(name, base_path, silence=silence)
                ...
                # INPUT DATA
                # We must ensure that the 'train' split is the first (for building the vocabulary)
                print("______________INPUT___________________")
                for split in ['train', 'val', 'test']:
                    if params['TEXT_FILES'].get(split) is not None:
                        if split == 'train':
                            build_vocabulary = True
                            ...
                            build_vocabulary = False
                        print(base_path + '/' + params['TEXT_FILES'][split] +target_l+"."+source_l)
                        ds.setInput(base_path + '/' + params['TEXT_FILES'][split] +target_l+"."+source_l,
                                    split,
                                    type=params.get('INPUTS_TYPES_DATASET', ['text', 'text'])[0],
                                    id=params['INPUTS_IDS_DATASET'][0],
                                    pad_on_batch=params.get('PAD_ON_BATCH', True),
...
def build_dataset_multilanguage(params):
